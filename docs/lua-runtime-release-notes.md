...
LuaSkills 0.5.1 additionally lets the host separate its LuaSkills data root, read-only managed Python/Node distribution root, and writable managed-environment root. Engine initialization accepts the B3-B7 managed-runtime policy with stable defaults of `4` Workers per exact environment/package-owner pool, `60` idle seconds, `256` persistent sessions, `1 MiB` per session output stream, and unlimited invoke time. Standard C ABI hosts use `FfiLuaRuntimeHostOptionsV3` plus the optional `FfiLuaRuntimeManagedRuntimeConfig` pointer; JSON FFI and language SDKs use `host_options.managed_runtime_config`.
...
三个 SDK 仓库还会发布 `scripts/deps/sync_runtime_assets.ps1` 与 `scripts/deps/sync_runtime_assets.sh`，作为 LuaSkills FFI、Lua runtime packages 与 VLDB 的统一直接同步入口。目标支持 `all`、`luaskills`、`lua`、`vldb`；数据库预设支持 `none`、`vldb-controller`、`vldb-direct`、`host-callback`。默认 LuaSkills 标签固定为 `v0.5.1`，受控验证时可显式覆盖发布标签。
